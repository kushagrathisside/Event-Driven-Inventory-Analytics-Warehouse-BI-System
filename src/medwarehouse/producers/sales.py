...
def _sample_sales_events(count: int, seed: str, product_id: str, warehouse_id: str):
    base_time = datetime(2026, 1, 1, 10, 0, tzinfo=timezone.utc)
    for index in range(count):
        event_time = base_time + timedelta(minutes=index * 7)
        sale_id = str(uuid5(NAMESPACE_URL, f"{seed}:sale:{index}"))
        yield {
            "event_id": str(uuid5(NAMESPACE_URL, f"{seed}:sales:{index}")),
...
def run_sales_producer(*, max_events: int | None = None, dry_run: bool | None = None) -> int:
    ...
    logger.info("Starting sales producer count=%s dry_run=%s", count, dry_run)
    return emit_events(
        topic=settings.kafka.sales_topic,
        events=_sample_sales_events(
            count=max(count, 1),
